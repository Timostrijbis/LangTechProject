...
def load_own_dataset(access_token):
    dataset = load_dataset('csv', data_files={'train': 'data/arguments_final.csv',
...
def train_model(tokenized_dataset, tokenizer, access_token, model_path):
    ...
    training_args = TrainingArguments(
        output_dir="deberta_v3_large_extra",
        learning_rate=1e-4,
        weight_decay=0.001,
        num_train_epochs=5,
        eval_strategy ="epoch",
    ...
    plt.savefig("results/deberta/extra/f1_scores_per_label.png")
    plt.close()
    ...
    plt.ylim(0, 1)
    plt.tight_layout()
    plt.savefig("results/deberta/extra/global_macro_micro_f1.png")
    plt.close()
...
def main():
    ...
    print("traing of model", model_path, "finished with extra data")
